...
**duckOSM automatically creates `config/{name}.yaml`** during this step.
...
- `config/tartu.yaml` — auto-generated by duckOSM (edit to customise modes, H3 resolutions, etc.)
- `output/tartu_{6,7,8}_h3_cells.geojson` — H3 boundary cells

### Step 2 — Fetch traffic (create a traffic config first)

Create `config/tartu_traffic.yaml` (copy from `config/traffic.template.yaml`):
...
**Auto-generated by duckOSM** the first time you run `pipeline_network.py` for an area.
